...
public class Dashboarddao {
    ...
    public int showUserCount() {
        String sql = "SELECT COUNT(email) AS user_count FROM customer";
        Integer count = this.template.queryForObject(sql, (rs, rowNum) -> rs.getInt("user_count"));
        return count != null ? count : 0;
    }
    public int showOrderCount() {
        String sql = "SELECT COUNT(status) AS user_count FROM customer";
        Integer count = this.template.queryForObject(sql, (rs, rowNum) -> rs.getInt("user_count"));
        return count != null ? count : 0;
    }

    public List<Coustmer> getAllCustomers() {
        ...
        template.query(sql, new RowMapper<Void>() {
            ...
            public Void mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                // Set Receipt details if they are available
                ...
                // Set ShirtDetails if they are available
    ...
    public Map<String, Float> getMonthlyRevenue() {
        String sql = "SELECT DATE_FORMAT(curr_date, '%Y-%m') AS month, SUM(amount) AS total_revenue " +
                     "FROM receipt " +
                     "GROUP BY month " +
                     "ORDER BY month";
